resource "proxmox_vm_qemu" "k8s-haproxy" {
  count             = 2
  name              = "tf-haproxy-0${count.index + 1}"
  target_node       = "pve"

  clone             = "ubuntu20LTS-cloudinit"
  full_clone	      = "1"
  force_create	    = false
  os_type           = "cloud-init"
  #network range 192.168.168.251 - 192.168.168.252
  ipconfig0         = "ip=192.168.168.25${count.index + 1}/24,gw=192.168.168.1"
  sshkeys           = "ssh-rsa  daro@windowsD"
  ciuser            = "root"
  onboot	          = "1"
  cores             = 1
  sockets           = "1"
  cpu               = "host"
  numa		          = "1"
  memory            = 1024
  scsihw            = "virtio-scsi-pci"
  bootdisk          = "scsi0"
  hotplug           = "network,cpu,memory,disk"
  agent		          = 1
  disk {
    #id              = 0
    size            = "10G"
    type            = "scsi"
    storage         = "vmstorage"
    storage_type    = "dir"
    format	        = "qcow2"
    iothread        = true
    cache	          = "writeback"
  }

  network {
    #id              = 0
    model           = "virtio"
    bridge          = "vmbr0"
    tag             = 0
  }
}
